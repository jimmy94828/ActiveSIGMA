...
def lerp_quaternions(start_quat, end_quat, max_angle_deg):
    ...
    # If there's effectively no rotation between the quaternions, return
    # the start quaternion and zero steps to avoid divide-by-zero.
    if total_angle_rad == 0 or total_angle_rad < 1e-12:
        return np.array([start_quat]), 0

    # Calculate the minimum steps required (at least 1)
...
def _safe_unit_vector(vec, fallback):
...
def _safe_rotation_matrix_from_direction(direction, gravity_vector):
    z_axis = _safe_unit_vector(direction, [0.0, 0.0, 1.0])
    y_ref = _safe_unit_vector(gravity_vector, [0.0, 1.0, 0.0])
    x_axis = np.cross(y_ref, z_axis)
    x_norm = np.linalg.norm(x_axis)
    if not np.isfinite(x_norm) or x_norm < 1e-8:
        alt_y = np.array([0.0, 1.0, 0.0], dtype=np.float64)
        if abs(float(np.dot(alt_y, z_axis))) > 0.95:
            alt_y = np.array([1.0, 0.0, 0.0], dtype=np.float64)
        x_axis = np.cross(alt_y, z_axis)
    x_axis = _safe_unit_vector(x_axis, [1.0, 0.0, 0.0])
    y_axis = _safe_unit_vector(np.cross(z_axis, x_axis), y_ref)
...
def calculate_initial_orientation(first_pos, second_pos):
    rotation_matrix = _safe_rotation_matrix_from_direction(
        np.asarray(second_pos) - np.asarray(first_pos),
        [0, -1, 0],
    )
...
def smoothen_trajectory(start_pose, end_pose, positions, max_angle_deg=10, gravity_vector=[0, 0, -1]):
    ...
    # Case 2: Positions > steps_for_180_deg * 2 (Positions should exceed rotations)
        ...
        # Calculate a robust trajectory-following rotation for the start direction.
        target_rotation_matrix = _safe_rotation_matrix_from_direction(
            positions[start_count + 1] - positions[start_count],
            gravity_vector,
        )
        ...
        # Calculate a robust trajectory-following rotation for the end direction.
        end_target_rotation_matrix = _safe_rotation_matrix_from_direction(
            positions[-end_count] - positions[-end_count - 1],
            gravity_vector,
        )
    ...
    # Case 3: Positions exceed rotations
        ...
        center_idx = min(start_count, len(interp_quats) - 1)
        center_rotation = R.from_quat(interp_quats[center_idx]).as_euler('xyz', degrees=True)
        mid_end = num_of_points - end_count
        if mid_end > start_count:
            rotations[start_count:mid_end] = center_rotation
