...
class ExplorationMap:
    def __init__(self, bounding_box, voxel_size, device='cpu', transform=None, use_xyz_filter = None, xy_sampling_step = None, gs_z_levels = None):
        ...
        self.last_changed_voxels = torch.empty((0, 3), dtype=torch.long, device=self.device)
    ...
    def update_from_depth_map(self, 
        ...
        if free_indices.numel() > 0:
            prev_vals = self.occupancy_grid[free_indices[:, 0], free_indices[:, 1], free_indices[:, 2]]
            changed_mask = prev_vals != -1.0
            if bool(changed_mask.any()):
                changed_parts.append(free_indices[changed_mask])
            self.occupancy_grid[free_indices[:, 0], free_indices[:, 1], free_indices[:, 2]] = -1.0

        if neglected_free_indices.numel() > 0:
            prev_vals = self.occupancy_grid[
                neglected_free_indices[:, 0], neglected_free_indices[:, 1], neglected_free_indices[:, 2]
            ]
            changed_mask = prev_vals != -2.0
            if bool(changed_mask.any()):
                changed_parts.append(neglected_free_indices[changed_mask])
            self.occupancy_grid[
                neglected_free_indices[:, 0], neglected_free_indices[:, 1], neglected_free_indices[:, 2]
            ] = -2.0
        ...
        if occupied_indices.numel() > 0:
            prev_vals = self.occupancy_grid[occupied_indices[:, 0], occupied_indices[:, 1], occupied_indices[:, 2]]
            changed_mask = prev_vals != 1.0
            if bool(changed_mask.any()):
                changed_parts.append(occupied_indices[changed_mask])
            self.occupancy_grid[occupied_indices[:, 0], occupied_indices[:, 1], occupied_indices[:, 2]] = 1.0

        if len(changed_parts) > 0:
            changed = torch.unique(torch.cat(changed_parts, dim=0), dim=0)
        else:
            changed = torch.empty((0, 3), dtype=torch.long, device=self.device)

        self.last_changed_voxels = changed
        return changed
    ...
    def get_free_voxels(self, use_xyz_filter: bool = True, xy_sampling_step: float = 1.0, gs_z_levels = None) -> torch.Tensor:
        ...
        if use_xyz_filter:
            ...
            # convert sampling step (meters) to integer voxel skip (at least 1)
            num_skip_vxl = max(1, int(round(float(xy_sampling_step) / float(self.voxel_size))))

            # build z-level mask
            if gs_z_levels is None:
                z_mask = torch.ones(free_voxels.shape[0], dtype=torch.bool, device=free_voxels.device)
            else:
                gs_z_levels_t = torch.tensor(gs_z_levels, dtype=free_voxels.dtype, device=free_voxels.device)
                z_mask = (free_voxels[:, 2].unsqueeze(1) == gs_z_levels_t.unsqueeze(0)).any(dim=1)

            # sample on XY grid using integer voxel skip
            xyz_mask = (
                (free_voxels[:, 0] % num_skip_vxl == 0)
                & (free_voxels[:, 1] % num_skip_vxl == 0)
                & z_mask
            )
            filtered_free_voxels = free_voxels[xyz_mask]
            return filtered_free_voxels
        else:
            return free_voxels
