...
def trilinear_interpolation(voxel_grid: np.ndarray, point: np.ndarray) -> np.ndarray:
    """trilinear interpolation; invalid/out-of-grid samples are treated as collision."""
...
def query_sdf_np(sdf_grid, points):
    ...
    sdf = np.asarray([trilinear_interpolation(sdf_grid, point) for point in points], dtype=np.float32)
    sdf[~np.isfinite(sdf)] = -np.inf
    return sdf
...
class RRT(PathPlanner):
    def __init__(self, 
        ...
        Args:
            ...
            reachable_batch_size (int): batch size for reachable-mask distance checks
        ...
        self.reachable_batch_size = max(1, int(reachable_batch_size))
    ...
    def get_reachable_mask(self) -> np.ndarray:
        ...
        ##################################################
        batch_size = self.reachable_batch_size
        invalid_mask = []

        for start in range(0, self.points.shape[0], batch_size):
            end = min(start + batch_size, self.points.shape[0])
            points2nodes = (self.points[start:end].reshape(-1, 1, 3) - self.nodes_tensor.reshape(1, -1, 3))
            ...
            invalid_mask.append(start + invalid_pt_idx)

        ### gather all invalid masks ###
        invalid_mask = torch.cat(invalid_mask) if invalid_mask else torch.empty(0, device=self._device, dtype=torch.long)
