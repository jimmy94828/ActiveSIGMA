#!/bin/bash
##################################################
### This script runs the full ActiveSIGMA system
...
ENABLE_VIS=${4:-0}
GPU_ID=${5:-0}
# bash scripts/activesigma/run_replica.sh room0 1 SemanticHeat 0 0,1
...
do
    for i in "${!seeds[@]}"; do
        ...
        python src/main/activesigma.py --cfg ${CFG} --setting ${SETTING} --seed ${seed} --result_dir ${result_dir} --enable_vis ${ENABLE_VIS}
