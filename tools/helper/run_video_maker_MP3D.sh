...
#
# Examples:
#   bash tools/helper/run_video_maker_MP3D.sh keyframe --scene GdvgFV5R1Z5
#   bash tools/helper/run_video_maker_MP3D.sh --version run_0_keyframe
#   bash tools/helper/run_video_maker_MP3D.sh v6 --fps 10 --dry-run
#   bash tools/helper/run_video_maker_MP3D.sh keyframe --scene GdvgFV5R1Z5 --python-bin /home/phudh/.conda/envs/visual/bin/python
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage:
  bash tools/helper/run_video_maker_MP3D.sh --version <version> [options]
  bash tools/helper/run_video_maker_MP3D.sh <version> [options]
...
for scene in "${SELECTED_SCENES[@]}"; do
  run_dir="$PROJ/results_MP3D/$DATASET/$scene/$METHOD/$RUN_NAME"
