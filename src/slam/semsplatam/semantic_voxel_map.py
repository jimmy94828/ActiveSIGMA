...
- Each voxel freezes a shared top-k class basis from its first observation.
...
class SemanticVoxelMap:
    def __init__(                # Initialize configuration parameters
        ...
        # The basis is selected from the voxel's first observation and then frozen.
    ...
    def _initialize_dir_classes(self, voxel_indices: Tensor, first_observations: Tensor) -> None:
        """Freeze each voxel's directional class basis from its first observation."""
        if voxel_indices.numel() == 0:
            return
        if first_observations.shape != (voxel_indices.numel(), self.n_classes):
            raise ValueError("first_observations must have shape [num_voxels, n_classes]")

        _, top_idx = torch.topk(first_observations, k=self.dir_topk, dim=-1)
        top_idx = torch.sort(top_idx, dim=-1).values
        self.dir_cls[voxel_indices] = top_idx.to(torch.int16)
    ...
    def update_from_frame(
        ...
        - Freezes each voxel's directional top-k basis from its first observation.
        ...
        first_observation = ~self.alpha_v_active[unique_v]
        self._initialize_dir_classes(
            unique_v[first_observation],
            q_v[first_observation],
        )
    ...
    def update_feasible_mask(
        ...
        check_depth: int = 3,    # probe steps along -d_p in exploration-grid voxel units
    ...
    def direction_bin_from_points(self, cam_pos_world: Tensor, points_world: Tensor) -> Tensor:
        """
        HEALPix binning using camera-to-point direction vectors.
        Direction definition: point - cam_pos, consistent with candidate semantic-gain queries.
        """
        d = points_world - cam_pos_world.view(1, 3)  # (N,3), camera -> voxel direction
