...
class SemSplatam(SplatamOurs):
    def __init__(self,
       ...
       '''
        ...
        self._feasible_mask_update_count = 0
    ...
    def eval_candidate_semantic_gains(self, c2w: torch.Tensor, seen: torch.Tensor = None) -> Tuple[float, float]:
            ...
            for v in visible_v.tolist():
                ...
                d = vox_center - cam_pos.to(device)
                denom = (d.norm() + 1e-8)
                d_norm = d / denom
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0 or force_map_update:
            ...
            if hasattr(self, 'semantic_voxel_map'):
                ...
                if bool(getattr(self.slam_cfg, 'sem_voxel_feasible_event_driven', False)):
                    ...
                    full_every = int(
                        getattr(self.slam_cfg, "sem_voxel_feasible_full_refresh_every", 0)
                    )

                    # Count actual occupancy-grid updates, not raw frame indices.
                    self._feasible_mask_update_count += 1
                    full_refresh = bool(
                        full_every > 0
                        and self._feasible_mask_update_count % full_every == 0
                    )
    ...
    def update_gs_map(self, 
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0 or force_map_update:
            ...
            with torch.no_grad():
                ...
                ##################################################
                use_gs3lam_keyframe_replay = bool(config['mapping'].get('use_gs3lam_keyframe_replay', True))
                opt_rskm_interval = max(1, int(config['mapping'].get('opt_rskm_interval', 5)))
                selected_keyframes = []
                opt_kf_sources = getattr(self.slam_cfg, 'opt_kf_sources', None)
                if opt_kf_sources is not None:
                    if isinstance(opt_kf_sources, str):
                        opt_kf_sources = [opt_kf_sources]
                    normalized_sources = []
                    for src in opt_kf_sources:
                        if not isinstance(src, str):
                            continue
                        key = src.strip().lower()
                        if key in ("local", "local_covisible", "covisible", "overlap"):
                            key = "local_covisible"
                        elif key in ("global", "global_keyframe"):
                            key = "global"
                        elif key in ("rskm", "gs3lam", "replay"):
                            key = "rskm"
                        else:
                            continue
                        if key not in normalized_sources:
                            normalized_sources.append(key)
                    if not self.slam_cfg.use_global_keyframe:
                        normalized_sources = [src for src in normalized_sources if src != "global"]
                    opt_kf_sources = normalized_sources if normalized_sources else None

                use_local_kf = ((opt_kf_sources is None) and (not use_gs3lam_keyframe_replay)) or \
                               ((opt_kf_sources is not None) and ("local_covisible" in opt_kf_sources))
                ...
                elif PRINT_INFO and opt_kf_sources is None:
                    replay_time_idx = [kf['id'] for kf in keyframe_list]
                    print(f"\nGS3LAM Replay Pool at Frame {time_idx}: {replay_time_idx}")
                    print(f"GS3LAM Replay Interval: every {opt_rskm_interval} mapping iters uses current frame")
                    if self.slam_cfg.use_global_keyframe:
                        global_keyframe_time_indices = [frame_idx for frame_idx in self.global_keyframe_time_indices if frame_idx != time_idx]
                        print(f"\nGlobal Keyframes at Frame {time_idx}: {global_keyframe_time_indices}")
                elif PRINT_INFO and opt_kf_sources is not None:
                    print(f"\nOptimization KF sources at Frame {time_idx}: {opt_kf_sources}")
            ...
            for iter in range(num_iters_mapping):
                ...
                ##################################################
                ### Overlap Keyframe / GS3LAM Replay ###

                if opt_kf_sources is not None:
                    if only_use_global_keyframe and ("global" in opt_kf_sources):
                        iter_source = "global"
                    else:
                        iter_source = np.random.choice(opt_kf_sources)

                    if iter_source == "global":
                        ### Global Keyframe ###
                    ...
                    elif iter_source == "local_covisible":
                        ...
                        # RSKM replay
                        use_current_frame = (time_idx == 0) or (len(keyframe_list) == 0) or (iter % opt_rskm_interval == 0)
                    ...
                    if only_use_global_keyframe or (self.slam_cfg.use_global_keyframe and iter > num_iters_mapping // 2):
                        ### Global Keyframe ###
                        # print(f"\nMapping Iteration {iter} at Frame {time_idx} using Global Keyframe Replay")
                        if len(self.global_keyframe_indices) == 1:
                            iter_time_idx = time_idx
                            iter_color = color
                            iter_depth = depth
                            ...
                            if 'crop_mask' in keyframe_list[selected_rand_keyframe_idx].keys():
                                ...
                                iter_crop_mask = get_uncert_mask(iter_seman,filter_pct=0.1,thres=self.slam_cfg['uncert_mask_thres'])
                                keyframe_list[selected_rand_keyframe_idx]['crop_mask'] = iter_crop_mask
                    elif use_gs3lam_keyframe_replay:
                        # GS3LAM RSKM-style replay: periodically optimize the current
                        # frame, and otherwise replay a uniformly sampled keyframe.
                        # print(f"\nMapping Iteration {iter} at Frame {time_idx}")
                        use_current_frame = (time_idx == 0) or (len(keyframe_list) == 0) or (iter % opt_rskm_interval == 0)
                        if use_current_frame:
                            iter_time_idx = time_idx
                            iter_color = color
