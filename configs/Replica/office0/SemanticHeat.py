...
if slam["method"] == "semsplatam":
    slam.update(
        ...
        bbox_bound = [[-2.1,2.5],[-3.2,2],[-1.3,2.0]], # x, y, z軸的範圍 (unit: m)
        # bbox_voxel_size = 0.05, # 大概有 660000個voxel
        bbox_voxel_size = 0.1,
        ...
        semantic_dir=os.path.join("data", "replica_v1", general["scene"][:-1] + "_" + general["scene"][-1], "habitat"),
...
##################################################
planner = dict(
    method= "semantic_heat",                           # planner method [predefined_traj, active_gs]
    # method = "predefined_traj",
    # method = "semantic_heat",

    ### active_gs params ###
    # gs_z_levels = [20, 30, 40, 50], #[20,30,40],
    # max_exploration_steps = 1500,
    max_exploration_steps = 595,
    ...
    ### NBV Score Filtering ###
    # Relative threshold: candidates scoring below (nbv_score_thre * best_score) are pruned.
    nbv_score_thre = 0.005,
...
if planner["local_planner_method"] == "RRTNaruto":
    planner.update(
        ...
        rrt_max_iter = 500,                         # maximum iterations for RRT (None → vol_size ≈654k which is too slow)
