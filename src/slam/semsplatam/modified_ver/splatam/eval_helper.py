...
voxel_heat_visualizer = VoxelHeatVisualizer()


def render_eval_voxel_heat_bgr(
    slam_model,
    color: torch.Tensor,
    depth: torch.Tensor,
    pose: torch.Tensor,
):
    slam_cfg = getattr(slam_model, "slam_cfg", None)
    if not bool(getattr(slam_cfg, "flag_debug_vis_eval_voxelheatmap", False)):
        return None

    semantic_voxel_map = getattr(slam_model, "semantic_voxel_map", None)
    if semantic_voxel_map is None or pose is None:
        return None

    color_for_vis = color
    if color_for_vis.ndim == 3 and color_for_vis.shape[0] in (1, 3, 4):
        color_for_vis = color_for_vis.permute(1, 2, 0)

    depth_for_vis = depth
    if depth_for_vis.ndim == 3 and depth_for_vis.shape[0] == 1:
        depth_for_vis = depth_for_vis[0]

    result = voxel_heat_visualizer.render(slam_model, color_for_vis, depth_for_vis, pose)
    return result.depth_aligned.heat_bgr
...
def plot_rgbds_silhouette_fast(color, depth, seman_rgb, rastered_color, rastered_depth,rastered_seman_rgb, presence_sil_mask, diff_depth_l1,
    ...
    row1_panels = [color_np, depth_np, diff_rgb_np if diff_rgb is not None else presence_sil_mask_np, seman_np]
    row2_panels = [rastered_color_np, rastered_depth_np, diff_depth_l1_np, rastered_seman_np]
    if voxel_heat_rgb is not None and voxel_heat_rgb.size > 0:
        voxel_heat_rgb = cv2.resize(voxel_heat_rgb, target_res, interpolation=cv2.INTER_NEAREST)
        row1_panels.append(np.zeros_like(voxel_heat_rgb, dtype=np.uint8))
        row2_panels.append(voxel_heat_rgb.astype(np.uint8))

    # Stack the images horizontally for row 1 and row 2
    row1 = np.hstack(row1_panels)
    row2 = np.hstack(row2_panels).astype(np.uint8)
...
def eval(slam_model, dataset, final_params, final_variables, num_frames, eval_dir, sil_thres,
    ...
    if save_frames:
        ...
        render_voxel_heat_dir = os.path.join(eval_dir, "rendered_voxel_heat")
        os.makedirs(render_voxel_heat_dir, exist_ok=True)
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        voxel_heat_bgr = render_eval_voxel_heat_bgr(
            slam_model,
            im,
            rastered_depth_viz,
            pose,
        ...
        voxel_heat_rgb = None
        if voxel_heat_bgr is not None and voxel_heat_bgr.size > 0:
            voxel_heat_rgb = cv2.cvtColor(voxel_heat_bgr, cv2.COLOR_BGR2RGB)
        ...
        if save_frames:
            ...
            if voxel_heat_bgr is not None and voxel_heat_bgr.size > 0:
                cv2.imwrite(os.path.join(render_voxel_heat_dir, "gs_{:04d}.png".format(time_idx)), voxel_heat_bgr)
    ...
    try:
        ...
        # Use GT exploration trajectory saved in params (active SLAM: explored frames != eval frames)
        if 'gt_w2c_all_frames' in final_params:
            gt_w2c_explore = [
                torch.from_numpy(final_params['gt_w2c_all_frames'][i]).cuda().float()
                for i in range(final_params['gt_w2c_all_frames'].shape[0])
            ]
            # Frame 0: estimated pose from params (initialized to GT, so ~identity in relative frame)
            cam0_rot = F.normalize(final_params['cam_unnorm_rots'][..., 0].detach())
            cam0_trans = final_params['cam_trans'][..., 0].detach()
            init_est_w2c = torch.eye(4).cuda().float()
            init_est_w2c[:3, :3] = build_rotation(cam0_rot)
            init_est_w2c[:3, 3] = cam0_trans
            latest_est_w2c_list = [init_est_w2c]
            valid_gt_w2c_list = [gt_w2c_explore[0]]
            for idx in range(1, min(num_frames, len(gt_w2c_explore))):
                # Skip frames with invalid GT
                if torch.isnan(gt_w2c_explore[idx]).sum() > 0:
                    continue
                ...
                valid_gt_w2c_list.append(gt_w2c_explore[idx])
        else:
            # Fallback: original behavior using eval dataset GT (may mismatch for active SLAM)
        ...
        ate_rmse = 100.0
        print(f'Failed to evaluate trajectory with alignment. Reason: {e}')
    ...
    np.savetxt(os.path.join(eval_dir, "frame_ids.txt"), np.array(frame_id_list), fmt="%d")
...
def eval_semantic(slam_model, dataset, final_params, final_variables, num_frames, eval_dir, sil_thres,
    ...
    if save_frames:
        seman_dir = os.path.join(eval_dir, "seman")
        os.makedirs(seman_dir, exist_ok=True)
        voxel_heat_dir = os.path.join(eval_dir, "voxel_heat")
        os.makedirs(voxel_heat_dir, exist_ok=True)
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Initialize Render Variables
        rendervar = transformed_params2rendervar(final_params, transformed_gaussians)
        depth_sil_rendervar = transformed_params2depthplussilhouette(final_params, curr_data['w2c'], transformed_gaussians)
        depth_sil, _, _, = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
        rastered_depth_viz = depth_sil[0, :, :].unsqueeze(0)
        ...
        miou_rp = calc_miou(pred=reprocess_ids, target=reprocess_pseudo_ids)
        ...
        if voxel_heat_rgb is not None and voxel_heat_rgb.size > 0:
            voxel_heat_rgb = cv2.resize(voxel_heat_rgb, target_res, interpolation=cv2.INTER_NEAREST)
            panels.append(voxel_heat_rgb.astype(np.uint8))

        final_image = np.hstack(panels).astype(np.uint8)
        save_path = os.path.join(plot_dir, f"{plot_name}.png")
        cv2.imwrite(save_path, cv2.cvtColor(final_image, cv2.COLOR_RGB2BGR))  # Convert RGB to BGR for OpenCV
        if save_frames and voxel_heat_bgr is not None and voxel_heat_bgr.size > 0:
            cv2.imwrite(os.path.join(voxel_heat_dir, f"{plot_name}.png"), voxel_heat_bgr)
    ...
    avg_miou_rp = np.array(miou_rp_list).mean()
    ...
    print("Average MIOU with Render vs Pseudo: {:.2f}".format(avg_miou_rp * 100))
...
def eval_semantic_mp3d(slam_model, dataset, final_params, final_variables, num_frames, eval_dir, sil_thres,
    ...
    plot_dir = os.path.join(eval_dir, "semantic_plots")
    os.makedirs(plot_dir, exist_ok=True)
    render_plot_dir = os.path.join(eval_dir, "plots")
    os.makedirs(render_plot_dir, exist_ok=True)
    if save_frames:
        render_rgb_dir = os.path.join(eval_dir, "rendered_rgb")
        os.makedirs(render_rgb_dir, exist_ok=True)
        render_depth_dir = os.path.join(eval_dir, "rendered_depth")
        os.makedirs(render_depth_dir, exist_ok=True)
        render_seman_dir = os.path.join(eval_dir, "rendered_semantic")
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        depth_chw = depth.permute(2, 0, 1) if depth.ndim == 3 else depth.unsqueeze(0)
        ...
        depth_sil, _, _, = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
        rastered_depth_viz = depth_sil[0, :, :].unsqueeze(0)
        valid_depth_mask = depth_chw > 0
        rastered_depth = rastered_depth_viz * valid_depth_mask
        silhouette = depth_sil[1, :, :]
        presence_sil_mask = silhouette > sil_thres
        ...
        weighted_im = im * valid_depth_mask
        weighted_gt_im = color_chw * valid_depth_mask
        psnr = calc_psnr(weighted_im, weighted_gt_im).mean()
        diff_depth_l1 = torch.abs(rastered_depth - depth_chw) * valid_depth_mask
        valid_depth_count = valid_depth_mask.sum().clamp_min(1)
        depth_l1 = diff_depth_l1.sum() / valid_depth_count
        ...
        gt_cls_id = curr_data['seman_gt'].detach().cpu().long().numpy()
        pseudo_cls_id = curr_data['seman_pseudo'].detach().cpu().long().numpy()
        rastered_cls_ids_np = rastered_cls_ids.detach().cpu().long().numpy()
        gt_seman_rgb = apply_colormap(gt_cls_id, sem_colormap)
        pseudo_seman_rgb = apply_colormap(pseudo_cls_id, sem_colormap)
        rastered_seman_rgb = apply_colormap(rastered_cls_ids_np, sem_colormap)
        ...
        iou_per_class_rp = calc_iou_per_classes(pred=rastered_cls_ids,
                                           target=curr_data['seman_pseudo'].long(),
                                           target_classes=target_classes)
        iou_g_list.append(iou_per_class_g)
        iou_p_list.append(iou_per_class_p)
        iou_rp_list.append(iou_per_class_rp)
        ...
        reprocess_pseudo_ids_np = reprocess_pseudo_ids.detach().cpu().long().numpy()
        recolored_pseudo_seman_rgb = apply_colormap(reprocess_pseudo_ids_np, sem_colormap)
        reprocess_ids_np = reprocess_ids.detach().cpu().long().numpy()
        recolored_rastered_seman_rgb = apply_colormap(reprocess_ids_np, sem_colormap)
        ...
        save_path = os.path.join(plot_dir, f"{plot_name}_gt.png")
        cv2.imwrite(save_path, cv2.cvtColor(gt_seman_rgb, cv2.COLOR_RGB2BGR))

        save_path = os.path.join(plot_dir, f"{plot_name}_render_miou_{miou_g_curr:.4f}.png")
        cv2.imwrite(save_path, cv2.cvtColor(recolored_rastered_seman_rgb, cv2.COLOR_RGB2BGR))

        prob_cls = next((cls_id for cls_id in target_classes if cls_id < rastered_seman.shape[-1]), None)
        if prob_cls is not None:
            prob_logits = rastered_seman[..., prob_cls]
            prob_logits = prob_logits.detach().cpu().numpy()
            heatmap_uint8 = np.uint8(255 * prob_logits)
            colored_heatmap = cv2.applyColorMap(heatmap_uint8, cv2.COLORMAP_JET)
            save_path = os.path.join(plot_dir, f"{plot_name}_prob_cls_{prob_cls}.png")
            cv2.imwrite(save_path, colored_heatmap)

        target_res = (256, 256)
        mode = 'nearest'
        gt_seman_rgb_t = torch.from_numpy(gt_seman_rgb / 255.).permute(2, 0, 1)
        gt_seman = resize_tensor(gt_seman_rgb_t, *target_res, mode=mode)
        gt_seman_np = gt_seman.permute(1, 2, 0).cpu().numpy() * 255

        pseudo_seman_rgb_t = torch.from_numpy(pseudo_seman_rgb / 255.).permute(2, 0, 1)
        pseudo_seman = resize_tensor(pseudo_seman_rgb_t, *target_res, mode=mode)
        pseudo_seman_np = pseudo_seman.permute(1, 2, 0).cpu().numpy() * 255

        recolored_pseudo_seman_rgb_t = torch.from_numpy(recolored_pseudo_seman_rgb / 255.).permute(2, 0, 1)
        recolored_pseudo_seman = resize_tensor(recolored_pseudo_seman_rgb_t, *target_res, mode=mode)
        recolored_pseudo_seman_np = recolored_pseudo_seman.permute(1, 2, 0).cpu().numpy() * 255

        rastered_seman_rgb_t = torch.from_numpy(rastered_seman_rgb / 255.).permute(2, 0, 1)
        rastered_seman_vis = resize_tensor(rastered_seman_rgb_t, *target_res, mode=mode)
        rastered_seman_np = rastered_seman_vis.permute(1, 2, 0).cpu().numpy() * 255

        recolored_rastered_seman_rgb_t = torch.from_numpy(recolored_rastered_seman_rgb / 255.).permute(2, 0, 1)
        recolored_rastered_seman = resize_tensor(recolored_rastered_seman_rgb_t, *target_res, mode=mode)
        recolored_rastered_seman_np = recolored_rastered_seman.permute(1, 2, 0).cpu().numpy() * 255
        ...
        if save_frames:
            ...
            vmin = 0.0
            vmax = float(getattr(getattr(slam_model.main_cfg, "visualizer", {}), "vis_rgbd_max_depth", 6.0))
            viz_render_depth = rastered_depth_viz[0].detach().cpu().numpy()
            normalized_depth = np.clip((viz_render_depth - vmin) / (vmax - vmin), 0, 1)
            depth_colormap = cv2.applyColorMap((normalized_depth * 255).astype(np.uint8), cv2.COLORMAP_JET)
            cv2.imwrite(
            ...
            cv2.imwrite(os.path.join(render_depth_dir, "gs_{:04d}.png".format(time_idx)), depth_colormap)
            cv2.imwrite(
                os.path.join(render_seman_dir, "gs_{:04d}.png".format(time_idx)),
                cv2.cvtColor(rastered_seman_rgb, cv2.COLOR_RGB2BGR),
            )

            viz_gt_im = torch.clamp(color_chw, 0, 1)
            viz_gt_im = viz_gt_im.detach().cpu().permute(1, 2, 0).numpy()
            viz_gt_depth = depth_chw[0].detach().cpu().numpy()
            normalized_depth = np.clip((viz_gt_depth - vmin) / (vmax - vmin), 0, 1)
            depth_colormap = cv2.applyColorMap((normalized_depth * 255).astype(np.uint8), cv2.COLORMAP_JET)
            cv2.imwrite(
    ...
    iou_rp_list = torch.stack(iou_rp_list)

    avg_iou_g_per_class = torch.nanmean(iou_g_list,dim=0)
    avg_iou_p_per_class = torch.nanmean(iou_p_list,dim=0)
    avg_iou_rp_per_class = torch.nanmean(iou_rp_list, dim=0)
    avg_mp3d8_miou_g = torch.nanmean(avg_iou_g_per_class).item() * 100
    avg_mp3d8_miou_p = torch.nanmean(avg_iou_p_per_class).item() * 100
    avg_mp3d8_miou_rp = torch.nanmean(avg_iou_rp_per_class).item() * 100
    ...
    for i in range(len(target_classes)):
        print(f"Render vs Pseudo: IoU for {target_classes_name[i]}: {avg_iou_rp_per_class[i].item()*100:.2f}")
    print(f"Average MIOU with GT (MP3D 8 classes): {avg_mp3d8_miou_g:.2f}")
    print(f"Average MIOU with Pseudo (MP3D 8 classes): {avg_mp3d8_miou_p:.2f}")
    print(f"Average MIOU with Render vs Pseudo (MP3D 8 classes): {avg_mp3d8_miou_rp:.2f}")
    print("Average MIOU with GT: {:.2f}".format(avg_miou_g * 100))
    print("Average MIOU with GT (current): {:.2f}".format(avg_miou_g_curr * 100))
    print("Average MIOU with Render vs Pseudo: {:.2f}".format(avg_miou_rp * 100))
    print("Average top1 acc with GT: {:.2f}".format(avg_top1_g * 100))
    ...
    with open(os.path.join(eval_dir, "IoU_per_classes.txt"), 'w') as f:
        ...
        for i in range(len(target_classes)):
            lines.append(f"iou_rp {target_classes_name[i]}: {avg_iou_rp_per_class[i].item()* 100}\n")
    ...
    with open(os.path.join(eval_dir, "semantic_result.txt"), 'w') as f:
        ...
        lines.append(f"mp3d8_miou_g: {avg_mp3d8_miou_g}\n")
        lines.append(f"mp3d8_miou_p: {avg_mp3d8_miou_p}\n")
        lines.append(f"mp3d8_miou_rp: {avg_mp3d8_miou_rp}\n")
        f.writelines(lines)
