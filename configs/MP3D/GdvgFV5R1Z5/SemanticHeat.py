...
if slam["method"] == "semsplatam":
    slam.update(
        ...
        # explore_map_iter = 1,
        refine_map_iter = 60,
        # use_global_keyframe = True,           # whether to use global keyframe for map refinement
        use_global_keyframe = True,     # Enable global keyframe usage during exploration & refinement
        skip_post_refinement_stage = True,  # If True, skip post_refinement even when use_global_keyframe=True
        # use_global_keyframe = False,          # whether to use global keyframe for map refinement
        ...
        semantic_dir=os.environ.get("ACTIVE_SIGMA_SEMANTIC_DIR", os.path.join("data", "MP3D", "v1", "scans", general["scene"])),
...
##################################################
planner = dict(
    method= "semantic_heat",                           # planner method [predefined_traj, active_lang]
    # method = "predefined_traj",

    ### active_lang params ###
    # gs_z_levels = [20, 30, 40, 50], #[20,30,40],
    # max_exploration_steps = 5000,
    max_exploration_steps = 1724,
    post_refine_steps = 0,
