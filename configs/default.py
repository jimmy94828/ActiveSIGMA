...
if planner["method"] == "naruto":
    ...
    if planner["local_planner_method"] == "RRTNaruto":
        planner.update(
            ...
            rrt_max_iter = None,                        # maximum iterations for RRT (None → vol_size ≈654k which is too slow)
            ...
            rrt_reachable_batch_size = 128,              # lower this if reachable-mask distance checks run out of VRAM
        )
...
if visualizer["method"] == "naruto":
    visualizer.update(
        ...
        save_voxel_heat     = True,                     # save voxel heat point cloud
        save_bev_diagnostic = True,                     # save per-iteration BEV diagnostic image
        bev_diagnostic_every = 1,                       # save BEV diagnostic every N iterations
        bev_diagnostic_size = 900,                      # BEV diagnostic canvas size in pixels
        bev_diag_jsd_top_percent = 0.10,               # overlay top 10% max directional-JSD voxels
        bev_diag_show_candidate_pool = True,           # overlay explore-pool candidates as gray dots
        bev_diag_pad_ratio = 0.05,                     # XY bound padding ratio for BEV framing
        bev_diag_remove_front_percent = 25.0,          # remove nearest visible top fraction to reveal indoor layout like bev_rgb
        bev_diag_flip_view = False,                    # use highest visible surface after front-removal
        bev_diag_min_sigma_px = 0.75,                  # minimum Gaussian sigma in pixels
        bev_diag_max_sigma_px = 3.0,                   # maximum Gaussian sigma in pixels
        bev_diag_max_kernel_radius = 4,                # Gaussian kernel radius cap in pixels
    )
