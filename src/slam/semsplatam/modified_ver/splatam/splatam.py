...
def calc_cosine(tensor1, tensor2, dim=0, return_mean=True, required_normalize=True, chunk_size=65536):
    eps = 1e-8
    if not required_normalize:
        cosine = F.cosine_similarity(tensor1, tensor2, dim=dim, eps=eps)
        return cosine.mean() if return_mean else cosine

    # Compute cosine similarity without materializing full normalized tensors.
    tensor1 = torch.movedim(tensor1, dim, 0)
    tensor2 = torch.movedim(tensor2, dim, 0)
    out_shape = tensor1.shape[1:]
    tensor1 = tensor1.reshape(tensor1.shape[0], -1)
    tensor2 = tensor2.reshape(tensor2.shape[0], -1)

    eps_sq = eps * eps
    if chunk_size is None or chunk_size <= 0 or tensor1.shape[1] <= chunk_size:
        dot = torch.einsum('cn,cn->n', tensor1, tensor2)
        norm1 = torch.sqrt(torch.einsum('cn,cn->n', tensor1, tensor1).clamp_min(eps_sq))
        norm2 = torch.sqrt(torch.einsum('cn,cn->n', tensor2, tensor2).clamp_min(eps_sq))
        cosine_flat = dot / (norm1 * norm2).clamp_min(eps)
    else:
        cosine_chunks = []
        total = tensor1.shape[1]
        for start in range(0, total, chunk_size):
            end = min(start + chunk_size, total)
            tensor1_chunk = tensor1[:, start:end]
            tensor2_chunk = tensor2[:, start:end]
            dot = torch.einsum('cn,cn->n', tensor1_chunk, tensor2_chunk)
            norm1 = torch.sqrt(torch.einsum('cn,cn->n', tensor1_chunk, tensor1_chunk).clamp_min(eps_sq))
            norm2 = torch.sqrt(torch.einsum('cn,cn->n', tensor2_chunk, tensor2_chunk).clamp_min(eps_sq))
            cosine_chunks.append(dot / (norm1 * norm2).clamp_min(eps))
        cosine_flat = torch.cat(cosine_chunks, dim=0)

    cosine = cosine_flat.reshape(out_shape)
    return cosine.mean() if return_mean else cosine
...
def calc_hellinger_distance(pred_dist, target_dist, eps=1e-8, chunk_size=65536):
    pred_dist = pred_dist.reshape(pred_dist.shape[0], -1)
    target_dist = target_dist.reshape(target_dist.shape[0], -1)

    if chunk_size is None or chunk_size <= 0 or pred_dist.shape[1] <= chunk_size:
        pred_dist = torch.clamp(pred_dist, min=0.0)
        target_dist = torch.clamp(target_dist, min=0.0)
        sqrt_pred = torch.sqrt(pred_dist)
        sqrt_target = torch.sqrt(target_dist)
        dist = torch.sqrt(0.5 * torch.sum((sqrt_pred - sqrt_target) ** 2, dim=0))
        return dist.mean()

    total = pred_dist.shape[1]
    dist_sum = pred_dist.new_tensor(0.0)
    dist_count = 0
    for start in range(0, total, chunk_size):
        end = min(start + chunk_size, total)
        pred_chunk = torch.clamp(pred_dist[:, start:end], min=0.0)
        target_chunk = torch.clamp(target_dist[:, start:end], min=0.0)
        sqrt_pred = torch.sqrt(pred_chunk)
        sqrt_target = torch.sqrt(target_chunk)
        dist_chunk = torch.sqrt(0.5 * torch.sum((sqrt_pred - sqrt_target) ** 2, dim=0))
        dist_sum = dist_sum + dist_chunk.sum()
        dist_count += dist_chunk.numel()

    return dist_sum / max(dist_count, 1)
...
def get_loss_with_seman(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
             ...
             lambda_hel = 0.8, lambda_cosine = 0.2, return_render_artifacts: bool = False):
    ...
    if return_render_artifacts:
        artifacts = {
            'seman_im': seman_im.detach(),
            'silhouette': silhouette.detach(),
            'seen': seen.detach(),
        }
        return loss, variables, weighted_losses, artifacts
