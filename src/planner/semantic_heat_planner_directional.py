...
Extends ActiveGSPlannerv2 with:
  ...
  2. Exploitation candidate branch – samples look-at poses near the
     top-ranked hot directions returned by SemanticVoxelMap, and injects them
...
class SemanticHeatPlanner(ActiveGSPlannerv2):
    ...
    hot_voxel_topk        : int   = 20    # legacy fallback for voxel-first budget
    hot_samples_per_voxel : int   = 3     # legacy fallback for per-voxel direction cap
    ...
    def __init__(
        ...
        self.hot_voxel_max_samples = int(self.planner_cfg.get("hot_voxel_max_samples", 0))
        self.hot_voxel_limit_reset_per_stage = bool(
            self.planner_cfg.get("hot_voxel_limit_reset_per_stage", False)
        )
        ...
        self.hot_direction_topk = int(
            self.planner_cfg.get(
                "hot_direction_topk",
                max(1, self.hot_voxel_topk * max(1, self.hot_samples_per_voxel)),
            )
        )
        self.hot_direction_max_per_voxel = int(
            self.planner_cfg.get(
                "hot_direction_max_per_voxel",
                max(1, self.hot_samples_per_voxel + max(0, self.hot_dir_random_extra)),
            )
        )
        self.hot_direction_score_w_dir = float(
            self.planner_cfg.get("hot_direction_score_w_dir", 1.0)
        )
        self.hot_direction_score_w_voxel = float(
            self.planner_cfg.get("hot_direction_score_w_voxel", 1.0)
        )
        self._hot_voxel_sample_count: Dict[int, int] = {}
        self._hot_voxel_sample_stage = int(getattr(self, "exploration_stage", 0))
    ...
    # ------------------------------------------------------------------
    # Exploitation: global top hot directions to candidate poses
    # ------------------------------------------------------------------

    def generate_exploitation_candidates(self, gs_slam) -> Optional[torch.Tensor]:
        """
        Sample look-at camera poses near the top-K hot directions.

        Each candidate direction is a global top-ranked ``(voxel, direction)``
        pair scored by a combination of directional heat ``H_v[p]`` and voxel
        value ``V(v)``. Candidate camera positions are placed along the chosen
        HEALPix direction at distance ``hot_sample_radius`` from the voxel centre.
        ...
        raw_k = max(
            self.hot_direction_topk,
            int(round(self.hot_direction_topk * self.hot_voxel_candidate_factor)),
        )
        hot_directions: List[Tuple[int, int, float]] = semantic_voxel_map.topk_hot_directions(
            raw_k,
            ...
            voxel_sample_count_dict=self._hot_voxel_sample_count if self.hot_voxel_max_samples > 0 else None,
            max_samples_per_voxel=self.hot_voxel_max_samples,
            per_voxel_cap=self.hot_direction_max_per_voxel,
            dir_weight=self.hot_direction_score_w_dir,
            voxel_weight=self.hot_direction_score_w_voxel,
        )
        if len(hot_directions) > self.hot_direction_topk:
            hot_directions = hot_directions[: self.hot_direction_topk]
        stats["hot_directions"] = len(hot_directions)
        if not hot_directions:
            stats["hot_directions_empty"] = True
            return None

        if self.hot_voxel_max_samples > 0 and self.hot_voxel_limit_reset_per_stage:
            if self._hot_voxel_sample_stage != int(self.exploration_stage):
                self._hot_voxel_sample_count.clear()
                self._hot_voxel_sample_stage = int(self.exploration_stage)
        ...
        for v, p_idx, _ in hot_directions:
            if self.hot_voxel_max_samples > 0:
                sampled_count = int(self._hot_voxel_sample_count.get(v, 0))
                if sampled_count >= self.hot_voxel_max_samples:
                    stats["reject_max_samples"] += 1
                    continue
            else:
                sampled_count = 0
            ...
            d_p = bin_dirs[p_idx]
            # Stage-1 sweep mode: search outward at fixed radii along the chosen hot direction.
            if self.exploration_stage == 1 and self.stage1_hot_radial_search:
                ...
                while radius <= r1 + 1e-8:
                    ...
                    if found_free and (not self.stage1_hot_collect_all_free):
                        break
            ...
            if self.hot_voxel_max_samples > 0:
                self._hot_voxel_sample_count[v] = sampled_count
    ...
    def rendering_based_planning(self, cur_pose, gs_slam):
        ...
        if (
            ...
            if self.exploration_stage >= 1:
                ...
                if expl_poses is None or expl_poses.shape[0] == 0:
                    ...
                    if isinstance(stats, dict) and len(stats) > 0:
                        ...
                        elif stats.get("hot_directions_empty", False):
                            reason = "hot_directions_empty"
                        else:
                        ...
                        self.info_printer(
                            ...
                            f"reason={reason} | "
                            f"hot_directions={int(stats.get('hot_directions', 0))} | "
                ...
                if expl_poses is not None and expl_poses.shape[0] > 0:
                    ...
                    expl_stats = getattr(self, "_last_exploitation_reject_stats", {})
                    ...
                    self.info_printer(
                        ...
                        f"from {int(expl_stats.get('hot_directions', 0))} hot directions into explore_pool.",
                        self.step, self.__class__.__name__,
                    )
            ...
            # ---- Score candidates (exploration not done yet) ----
                ...
                # score_norm_low_pct = float(self.planner_cfg.get("score_norm_low_pct", 10.0))        # NBV score normalization low percentile for robust scaling (ignore outliers)
                # score_norm_high_pct = float(self.planner_cfg.get("score_norm_high_pct", 90.0))      # NBV score normalization high percentile for robust scaling (ignore outliers)
                ...
                '''if (gi_t.max() - gi_t.min()) > 1e-6:
                    ...
                    gi_norm = torch.zeros_like(gi_t)'''
                    
                gi_norm = self.minmax_normalize(gi_t)

                '''if (gd_t.max() - gd_t.min()) > 1e-6:
                ...
                # Re-validate only the selected HOT candidate to avoid
                # selecting near-obstacle / unknown poses from exploitation.
                if self.hot_selection_revalidate:
                    ...
                    def _hot_pose_safe(start_slam: torch.Tensor, end_slam: torch.Tensor) -> bool:
                        ...
                        def _idx_ok(xi: int, yi: int, zi: int) -> bool:
                            if (
                                ...
                                return False
                            if margin <= 0:
                                return float(occ[xi, yi, zi].item()) <= occ_th
                            neigh = occ[
                                xi - margin : xi + margin + 1,
                                yi - margin : yi + margin + 1,
                                zi - margin : zi + margin + 1,
                    ...
                    chosen_idx = int(sorted_idx[0].item())
                    masked_hot = 0
                    ...
                    next_visit = chosen_idx
                else:
                    next_visit = int(torch.argmax(weighted_score).item())
